...
class AuthorForm(FlaskForm):
    ...
    validators=[
        DataRequired(), 
        Length(min=3, max=80, message="Author's name needs to be betweeen 1 and 80 chars")
    ])

    # - the author's biography (hint: use a TextAreaField)
    biography = TextAreaField('Biography', 
    validators=[
        Length(min=3, max=80, message="Biography needs to be betweeen 1 and 80 chars")
    ])
